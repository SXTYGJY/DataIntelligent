...
class TestIngestionPipeline:
    ...
    def test_pipeline_skip_already_processed(self, settings, simple_pdf_path):
        ...
        try:
            ...
            assert result2.stages["integrity"].get("skipped")
    ...
    def test_pipeline_force_reprocess(self, settings, simple_pdf_path):
        ...
        try:
            ...
            assert not result2.stages.get("integrity", {}).get("skipped")
...
class TestPipelineComponents:
    ...
    def test_settings_loads_correctly(self, settings):
        ...
        assert settings.vision_llm.enabled
        ...
        assert settings.ingestion.chunk_refiner.get("use_llm")
        assert settings.ingestion.metadata_enricher is not None
        assert settings.ingestion.metadata_enricher.get("use_llm")
